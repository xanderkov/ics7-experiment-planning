...
class Experiment():
    ...
    def denormalize(self, x, xmin, xmax):
    ...
    def expand_plan(self, plan, y, xmat):
        ...
        for i in range(len(xmat)):
            ...
            for j in range(len(xmat[i])):
                b_cur += xmat[i][j] * y[j]
            b.append(b_cur)
            # self.denormalize(b_cur, self.min_gen_int, self.max_gen_int )
            # b.append(b_cur/MATR_SIZE)
        b[1] = self.denormalize(b[1], self.min_gen_int, self.max_gen_int) * np.sign(b[1])
        b[2] = self.denormalize(b[2], self.min_pm_int, self.max_pm_int) * np.sign(b[2])
        b[3] = self.denormalize(b[3], self.min_gen_var, self.max_gen_var) * np.sign(b[3])
        b[4] = self.denormalize(b[3], self.min_pm_var, self.max_pm_var) * np.sign(b[4])

        b[5] = self.denormalize(b[5], self.min_gen_int * self.min_pm_int, self.max_gen_int * self.max_pm_int)  * np.sign(b[5])
        b[6] = self.denormalize(b[6], self.min_gen_int * self.min_gen_var, self.max_gen_int * self.max_gen_var)  * np.sign(b[6])
        b[7] = self.denormalize(b[7], self.min_gen_int * self.min_pm_var, self.max_gen_int * self.max_pm_var)  * np.sign(b[7])
